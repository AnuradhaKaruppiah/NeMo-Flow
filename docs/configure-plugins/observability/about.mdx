...
trajectory root scope UUID as `trajectory_id` and, by default, as `session_id`.
Set the ATIF `session_id_source` to `propagation_root` to use the propagated root
UUID as the run-scoped `session_id` while retaining the trajectory scope UUID as
`trajectory_id`. ATIF step lineage stores the event UUID as
...
storage keys remain based on the trajectory scope UUID under either
`session_id_source` setting. Logical coding-session values also remain available
under `extra.nemo_relay`.
