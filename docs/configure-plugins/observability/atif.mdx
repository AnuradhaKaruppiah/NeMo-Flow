...
remote session header remain trajectory-unique artifact identities. Configure
`session_id_source = "propagation_root"` when the serialized ATIF `session_id`
should instead identify the propagated run.
...
| `session_id_source` | `agent_scope` | Source for the serialized root ATIF `session_id`: `agent_scope` preserves the trajectory scope UUID, while `propagation_root` uses that scope's parent/root UUID. |
...
| `filename_template` | `nemo-relay-atif-{session_id}.json` | Must contain the historical `{session_id}` placeholder, which expands to the trajectory scope UUID regardless of `session_id_source`. Can contain `{metadata.<path>}` placeholders for metadata-based routing. |
...
Set `session_id_source = "propagation_root"` when the propagation root carries a
...
For a top-level Agent scope, Relay then emits the propagation root as the ATIF
...
UUID so multiple trajectories in one run cannot overwrite each other. The
default `agent_scope` setting preserves the historical behavior where
`session_id` and `trajectory_id` are both the Agent scope UUID.
...
top-level trajectory root scope UUID by default, or the propagated root UUID
when `session_id_source = "propagation_root"`. The ATIF `trajectory_id` remains
