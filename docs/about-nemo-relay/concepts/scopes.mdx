...
context contains an immediate `parent_uuid` and a `root_uuid`. By default,
Relay uses the propagated root or the first local Agent scope as that root.
...
and OpenTelemetry trace continuity. Use
